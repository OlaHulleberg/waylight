...
pub const DesktopWatcher = struct {
    ...
    fn addWatch(self: *DesktopWatcher, path: []const u8) !void {
        ...
        const wd = try posix.inotify_add_watch(
            ...
            linux.IN.CREATE | linux.IN.DELETE | linux.IN.MODIFY | linux.IN.MOVED_FROM | linux.IN.MOVED_TO | linux.IN.CLOSE_WRITE,
        );
    ...
    fn processEvents(self: *DesktopWatcher) void {
        ...
        while (true) {
            ...
            while (offset < n) {
                ...
                if (event.len > 0) {
                    const name_ptr: [*:0]const u8 = @ptrCast(&event.name);
